								...
								<div class="row">
									...
									<div class="col-md-6">
										<div class="form-group" :class="{'has-error' : errors.has('form.comment')}">
											...
											<!-- text -->
											<input type="text" name="comment" class="form-control" placeholder="Silahkan masukkan Comment" v-validate="'required|min:5'" data-vv-as="comment" v-model="form.comment">

											<!-- error message -->
											<small class="text-muted text-danger" v-if="errors.has('form.comment')">
												<i class="icon-arrow-small-right"></i> {{ errors.first('form.comment') }}
											</small>
											<small class="text-muted" v-else>&nbsp;</small>
										</div>
									</div>
