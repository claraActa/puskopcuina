		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl"  :isNoButtonRow="isNoButtonRow" :dataview="dataview" @fetch="fetch">
			...
			<template slot="button-desktop">
				...
				<template v-if="$route.params.tipe == 'diklat_bkcu'">
					...
					<!-- formulir-->
					<button @click.prevent="formPendaftaran2(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['index_diklat_bkcu']" :disabled="!selectedItem.id">
						<i class="icon-clipboard6"></i> Formuir Pendaftaran
					</button>
				</template>
				...
				<template v-else-if="$route.params.tipe == 'pertemuan_bkcu'">
					...
					<button @click.prevent="formPendaftaran2(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['index_pertemuan_bkcu']" :disabled="!selectedItem.id">
						<i class="icon-clipboard6"></i> Formulir Pendaftaran
					</button>
				</template>
			...
			<template slot="button-mobile">
				...
				<template v-if="$route.params.tipe == 'diklat_bkcu'">
					...
					<!-- form -->
					<button @click.prevent="formPendaftaran2(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['index_diklat_bkcu']" :disabled="!selectedItem.id">
						<i class="icon-clipboard6"></i> Formulir Pendaftaran
					</button>
				</template>
				...
				<template v-else-if="$route.params.tipe == 'pertemuan_bkcu'">
					...
					<!-- form Pendaftaran -->
					<button @click.prevent="formPendaftaran2(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['index_pertemuan_bkcu']" :disabled="!selectedItem.id">
						<i class="icon-clipboard6"></i> Formulir Pendaftaran
					</button>
				</template>
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			...
			formPendaftaran2(id) {
				this.$router.push({name: this.kelas + 'formPendaftaran', params: { id: id }});
			},
