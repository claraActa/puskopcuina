							...
							<div class="card">
								...
								<div class="card-body">
									...
									<template v-if="item.tipe == 'diklat_bkcu'">
										...
										<!-- formulir daftar -->
										<template v-if="item.status != 5 && item.status != 6">
											<button class="btn bg-warning-400 btn-block mb-2" @click.prevent="formulirPendaftaran(selectedItem.id)" v-if="currentUser.id_cu == 0">
												<i class="icon-clipboard6"></i> Formulir Pendaftaran Diklat
											</button>

											<!-- formulir daftar -->
											<button class="btn bg-warning-400 btn-block mb-2" @click.prevent="formulirPendaftaran(selectedItem.id)" v-else-if="currentUser.can && currentUser.can['index_diklat_bkcu'] && currentUser.id_cu != 0 && item.status == 2">
												<i class="icon-clipboard6"></i> Formulir Pendaftaran Diklat
											</button>
										</template>
										...
										<!-- formulir daftar -->
										<template v-if="item.status != 5 && item.status != 6">
											<button class="btn bg-warning-400 btn-block mb-2" @click.prevent="formUmum(selectedItem.id)" v-if="currentUser.id_cu == 0">
												<i class="icon-clipboard6"></i> Formulir Pendaftaran Umum
											</button>

											<!-- formulir daftar -->
											<button class="btn bg-warning-400 btn-block mb-2" @click.prevent="formUmum(selectedItem.id)" v-else-if="currentUser.can && currentUser.can['index_diklat_bkcu'] && currentUser.id_cu != 0 && item.status == 2">
												<i class="icon-clipboard6"></i> Formulir Pendaftaran Umum
											</button>
										</template>
									</template>
...
<script>
	...
	import formUmum from "./formUmum.vue";
	import formulirPendaftaran from "./formulirPendaftaran.vue";
	...
	export default {
		...
		methods: {
			...
			downloadMateri(filename){
	  		...
	  		formUmum() {
				this.$router.push({name: 'kegiatanBKCUformUmum'});
			},

			formulirPendaftaran(){
				this.$router.push({name: 'kegiatanBKCUformulirPendaftaran'})
			},
